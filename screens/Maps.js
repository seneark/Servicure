...
const Maps = () => {
  ...
  const getTrack = async () => {
    ...
    for (let i = 0; i < arr.length; i++) {
      await rdb
        .ref("/location " + arr[i].phone.toString())
        .on("value", (snapshot) => {
          if (snapshot.val()) {
            let obj = distData;
  ...
  return (
    ...
    >
      <Block flex center>
        <StatusBar hidden />
        <Modal
          onModalHide={openSettings ? openSetting : undefined}
        ...
        {isLoading ? (
          ...
          <Block>
            ...
            <Block>
              ...
              <Text style={{ color: "rgba(200,200,0,1)" }}>
                Can expect a delay
              </Text>
              <Text style={{ color: "rgba(0,0,255,0.8)" }}>
                Blue: No Delay in Traffic but there is a emergency
              </Text>
            </Block>
          </Block>
        )}
